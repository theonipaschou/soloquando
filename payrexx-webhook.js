// api/payrexx-webhook.js
// Receives Payrexx payment webhooks and increments the order count
// in the matching batch-NNN-count.json via the GitHub API.
//
// Handles two purpose formats:
//   "Batch 002 - Mousto - 3 pieces"              → 3 units
//   "Batch 004 - VITA.RITUAL Daily Box - 2 x Trio (3 balls)"  → 2 units
//
// Required environment variables in Vercel:
//   GITHUB_TOKEN    — Personal Access Token with repo write scope
//   GITHUB_OWNER    — your GitHub username or org
//   GITHUB_REPO     — repository name
//   WEBHOOK_SECRET  — secret string appended to the webhook URL
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 5. Extract quantity ───────────────────────────────────────────────
    // Format A: "3 pieces"  (batch 002)
    // Format B: "2 x Trio (3 balls)"  (batch 004)
    let qty = 1;
    ...
    console.log(`Batch: ${batchId}, quantity: ${qty}, file: ${countFile}`);

    // ── 6. Read current count from GitHub ─────────────────────────────────
    ...
    // ── 7. Increment, clamp to max, update status ─────────────────────────
    ...
    console.log(`Count: ${oldReserved} → ${newReserved} (status: ${newStatus})`);
    ...
    console.log(`Done: batch ${batchId} now at ${newReserved}`);
