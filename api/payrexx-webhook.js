module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const purpose = body['transaction[invoice][number]'] || '';

    console.log(`Webhook: status=${status} purpose="${purpose}"`);
    ...
    if (!batchMatch) {
      console.warn('Could not parse batch from:', purpose);
    ...
    if (piecesMatch)     qty = parseInt(piecesMatch[1], 10);
    else if (boxesMatch) qty = parseInt(boxesMatch[1], 10);
    ...
    return res.status(200).json({ ok: true, batch: batchId, qty, reserved: newReserved });
